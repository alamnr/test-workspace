...
class HelloMongoDBTestcontainerNTest {
    @Container
    private static MongoDBContainer mongoDB = new MongoDBContainer("mongo:4.4.0-bionic");
//MongoDBContainer cannot be used with authentication added. The internals of the DB-specific image
// configures it for a single-node ReplicaSet in order to support transactions. The problem is the configuration
//to do so is issued without credentials -- causing security-enabled mongoDB to reject the command and the
//container to fail to start. If you want security enabled, you have to use the GenericContainer.
//I am going to use the MongoDBContainer for this example to show it can be configured to work.
...
to activate security and require user credentials.
    private static GenericContainer mongoDB = new GenericContainer("mongo:4.4.0-bionic")
            .withEnv("MONGO_INITDB_ROOT_USERNAME", "admin")
            .withEnv("MONGO_INITDB_ROOT_PASSWORD", "secret")
            .withEnv("MONGO_INITDB_DATABASE","testcontainers")
            .withExposedPorts(27017);
*/
    @DynamicPropertySource
    private static void addLateSpringContextProperties(DynamicPropertyRegistry registry) {
        registry.add("spring.data.mongodb.uri",()-> mongoDB.getReplicaSetUrl("testcontainers"));
    }
    ...
    void can_populate_spring_context_with_dynamic_properties() {
        then(expectedMongoUrl).matches("mongodb://.*(?:localhost|host.docker.internal):[0-9]+/testcontainers");
    }
