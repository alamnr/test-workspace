...
public class BooksAnnotatedQueryMethodsNTest {
    ...
    void findByBetween() {
        ...
        //when
        Page<Book> foundBookPage = booksRepository.findByPublishedBetween(firstBook.getPublished(), lastBook.getPublished(), pageable);
        //then
        then(foundBookPage).hasSize(1);
        then(foundBookPage.getTotalElements()).isEqualTo(foundIds.size());
    }
